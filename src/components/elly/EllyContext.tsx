...
type EllyCtx = { open: boolean; openElly: () => void; closeElly: () => void };
...
export function EllyProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <Ctx.Provider value={{ open, openElly: () => setOpen(true), closeElly: () => setOpen(false) }}>
      {children}
      {open && <EllyPortal open={open} onClose={() => setOpen(false)} />}
    </Ctx.Provider>
  );
}
